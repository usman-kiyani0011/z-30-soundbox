...
/* Demo helpers for offline QR showcase */
static char g_demo_amount[] = "123.45";
static char g_demo_qr[] = "https://demo.nez30/payment/12345";

static void demo_play_amount_voice(const char *amount)
{
	long long money = 0;
	int wait_count = 0;
	...
	money = MfSdkUtilStr2Longlong((const s8 *)amount);
	if (money <= 0) {
		return;
	}
	...
	if (strlen(amount) < 16) {
		...
		strncpy(display_buf, amount, sizeof(display_buf) - 1);
		MfSdkGuiLedAmount(display_buf);
		amt_time = MfSdkSysGetTick();
	}

	while (MfSdkAudTtsState() == 1 && wait_count++ < 30) {
		MfSdkSysSleep(100);
	}
...
static void demo_trigger_qr(void)
{
	...
	APP_TRACE("[demo] trigger QR showcase\r\n");
	show_page_qrcode(parent, g_demo_amount, g_demo_qr);
	demo_play_amount_voice(g_demo_amount);
}
