...
static void wifi_task_func(lv_task_t* task)
{
	int index = 0;
...
void state_wifi_page_init(void* state, lv_align_t align, lv_coord_t x_ofs, lv_coord_t y_ofs)
{
	...
	lv_task_create(wifi_task_func, 1000, LV_TASK_PRIO_MID, NULL);
	return;
}
...
static void atc_task_func(lv_task_t* task)
{
	int index = 0;
...
void state_atc_page_init(void* state, lv_align_t align, lv_coord_t x_ofs, lv_coord_t y_ofs)
{
	...
	lv_task_create(atc_task_func, 1000, LV_TASK_PRIO_MID, NULL);
	return;
}
...
static void power_task_func(lv_task_t* task)
{
	...
	if (power_index != index || power_status != status) 
	{
		power_index = index;
		power_status = status;
		lv_img_set_src(img_power, get_power_img(index, 0));
	}
	return;
}
