...
void page_list_close_page(int ret)
{
	_list_close_page(ret);
}
...
static void _list_btn_click_comfirm(lv_obj_t* page_list, lv_obj_t* btn)
{
	int index = 0;
    if(page_list == NULL) { return ; }
	...
	if (btn != 0) 
	{
		index = lv_list_get_btn_index(page_list, btn);
		cell_selected_index = -1;
		if (m_page_close_page_func != 0) m_page_close_page_func(index, list_body);
	}
}
...
static void _list_win_event_comfirm_cb(struct _lv_obj_t* obj, lv_event_t event)
{
	lv_obj_t* page_list = NULL;
	lv_obj_t* btn;
	int index;
	...
	if (event == LV_EVENT_KEY) 
	{
		...
		if (key == MF_LV_KEY_OK_SHORT_PRESS) 
		{
			...
			btn = lv_list_get_btn_selected(page_list);
			APP_TRACE("lv_list_get_btn_selected\r\n");
			_list_btn_click_comfirm(page_list, btn);
		}
		else if (key == MF_LV_KEY_F2_SHORT_PRESS) 
		{
			btn = lv_list_get_btn_selected(page_list);
...
static lv_obj_t* _list_btn_add(lv_obj_t* list_page, char* text, int index)
{
	...
	lv_obj_set_style_local_text_font(btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_FONT_24);
...
lv_obj_t* page_list_show_ex(lv_obj_t* parent, void* pfunc, char* title, char* listitem[], int count, int index, int timeover, int show_back)
{
	...
	lv_obj_set_size(page_list, lv_obj_get_width(page_win), lv_obj_get_height(page_win) - page_get_title_height()-page_get_statusbar_height());
	lv_obj_set_user_data(page_win, (lv_obj_user_data_t)page_list);
	lv_obj_align(page_list, page_win, LV_ALIGN_IN_TOP_LEFT, 0, page_get_title_height()+page_get_statusbar_height());
