...
static void _message_event_cb(lv_obj_t* obj, lv_event_t e)
{
    ...
    if (e == LV_EVENT_KEY) {
        ...
        if (key == MF_LV_KEY_CANCEL_SHORT_PRESS) {
            if(get_http_run_flag())//If http is started, the http thread is used to shut down and release resources
            {
        	    //lv_label_set_text(lab_message, "Exiting, please wait");
                page_spinner = create_spinner(lv_scr_act(), "Tip", "quit,please wait...");
                set_http_exist_flag(HTTP_CLOSE);
            }
            else//If the system does not start, resources are released
            {
...
static void _message_task_func(lv_task_t* task)
{
	...
	if (m_timeover > 0) {
        ...
        sprintf(countdown, "(%ds)", m_timeover / 1000);
...
lv_obj_t* page_http_message_show_ex(lv_obj_t* parent, void* pfunc, char* title, char* message, char* leftbtn, char* rightbtn, int timeover,int show_back)
{
    ...
    lv_label_set_long_mode(lab_message, LV_LABEL_LONG_BREAK);
    //lv_label_set_recolor(lab, true);
    lv_label_set_text(lab_message, message);
    lv_obj_set_style_local_bg_color(lab_message, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_WHITE);
    lv_obj_set_style_local_text_font(lab_message, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_FONT_24);
    lv_obj_set_style_local_text_color(lab_message, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
    lv_obj_set_width(lab_message, lv_obj_get_width(parent));
    lv_obj_align(lab_message, NULL, LV_ALIGN_CENTER, 0, 0);

    char countdown[32] = { 0 };
    lab_countdown = lv_label_create(page_win, NULL);
    //lv_obj_set_style_local_text_font(btn1, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, &lv_font_montserrat_16);
    sprintf(countdown, "(%ds)", timeover / 1000);
    lv_label_set_text(lab_countdown, countdown);
    lv_obj_set_style_local_text_color(lab_countdown, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x000000));
    lv_obj_set_size(lab_countdown, 60, 30);
    lv_obj_align(lab_countdown, page_win, LV_ALIGN_IN_BOTTOM_MID, 0, -30);
