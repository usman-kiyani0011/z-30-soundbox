...
lv_obj_t* page_message_show_align(lv_obj_t* parent, void* pfunc, char* title, char* message, char* leftbtn, char* rightbtn, int timeover,int align)
{
    int height = 0;
	lv_font_t* font = LV_FONT_16;
	if(parent == NULL){
		APP_TRACE("page_message_show_ex error is null title:%s\r\n",title);
		return NULL;
	}
    ...
    lab_message = page_create_msg_align(page_win, message, font, align);
...
static void _message2_event_cb(lv_obj_t* obj, lv_event_t e)
{
	uint32_t key;
	int dist = 62;
    ...
    if (e == LV_EVENT_KEY) 
	{
		...
		else if (key == MF_LV_KEY_OK_SHORT_PRESS || key == MF_LV_KEY_FUNC_SHORT_PRESS || key == MF_LV_KEY_F1_SHORT_PRESS) 
		{
...
lv_obj_t* page_message_show(lv_obj_t* parent , void * pfunc , char * title, char * message ,char * leftbtn, char *rightbtn, int timeover)
{
	...
	page_win = page_create_win(parent, _message2_event_cb);
	...
	lab_message = page_create_msg_ex(page_win, message, font);
