...
#else
/* Code 128 and NVE-18 */
extern int code_128(struct zint_symbol *symbol, uint8_t *source, int length);
#endif
...
int test_code_128(struct zint_symbol *symbol, uint8_t *source, int length)
{
	code_128(symbol, source, length);
...
int reduced_charset(struct zint_symbol *symbol, uint8_t *source, int length)
{
	...
	uint8_t *preprocessed = (uint8_t *)MALLOC(length + 1);
...
#else
		case BARCODE_CODE128: error_number = test_code_128(symbol, preprocessed, length); break;
		case BARCODE_CODE128B: error_number = test_code_128(symbol, preprocessed, length); break;
#endif
	}
	FREE(preprocessed);
	return error_number;
...
int ZBarcode_Encode(struct zint_symbol *symbol, uint8_t *source, int length)
{
	...
	local_source = (uint8_t *)MALLOC(length + 1);
