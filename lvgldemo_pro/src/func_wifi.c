...
static MfSdkCommWifiApList_T wifi_ap_list[MAX_AP_COUNT+1] = { 0 };
...
static void delete_wifi_task()
{
	...
	wifi_process = 0;
	AppPowerUnlockApp((char*)"func_wifi_show");
	lv_start_lock(1);
	_spinner_close_page();
	lv_start_lock(0);
...
static int _wifi_list_select_func(int ret,lv_obj_t* obj)
{
	...
	if (mbox_wifi == NULL) {
		...
		if((ret>=0)&&(strcmp(wifi_ap_list[ret].ssid,(char*)MfSdkCommWifiGetSsid()) == 0) && MfSdkCommWifiGetLinkState()== CONNECTED){
			APP_TRACE("MfSdkCommWifiGetSsid():%s,MfSdkCommWifiGetLinkState():%d",(char*)MfSdkCommWifiGetSsid(),MfSdkCommWifiGetLinkState());
			#if 1
			if (mbox_wifi == NULL)
			{
				char msg_buff[100] = { 0 };
...
static void _wifi_task(void* task)
{
	...
	wifi_ap_count = MfSdkCommWifiListAp(wifi_ap_list);
	APP_TRACE("MfSdkCommWifiListAp:%d\r\n", wifi_ap_count);
	if (wifi_ap_count <= 0)
	{ 
	...
	while (MfSdkSysCheckTick(tick1, 30000) == 0) {		// wait scan
		...
		if (exit_process) {
			APP_TRACE("exit_process MfSdkCommWifiListAp exit\r\n");
			delete_wifi_task();
			lv_start_lock(1);
	...
	while (select_index == 999){		// wait select ap
		...
		if (get_refresh_wifi_list_flag() == 1) {
			...
			APP_TRACE("MfSdkCommWifiListAp again");
			memset(&wifi_ap_list, 0x00, sizeof(st_wifi_ap_list)*(MAX_AP_COUNT+1));
			wifi_ap_count = MfSdkCommWifiListAp(wifi_ap_list);
			#if 1 
			for (int i = 0; i < wifi_ap_count - 1; i++)
...
static int page_return_wifi_func(int ret,lv_obj_t* obj)
{
	...
	exit_scan_page();
	create_spinner(lv_scr_act(), "Wifi list", "quit,please wait...");
	return 1;
}
