...
int MqttPlatformInit()
{
    ...
    if (strlen((char*)g_mqttConfig.mqPassword) < 3)
    {
		strcpy((char*)g_mqttConfig.mqPassword, "");  // Empty password if not configured
    }
...
static int mqtt_comm_run()
{
	...
	//Note: HiveMQ Cloud requires TLS but this implementation uses TLS without client certificates
	rc = NetworkConnect_ssl(&n, g_mqttConfig.ip, g_mqttConfig.port, "", "", "");
	APP_TRACE("rc from net:%d\r\n", rc);
    if ( rc != 0 ) 
	{
	...
	//Set up mqtt connection data for HiveMQ Cloud
	MQTTPacket_connectData data = MQTTPacket_connectData_initializer;
	data.cleansession = 0;  // Enable persistent session to receive offline messages
	data.keepAliveInterval = g_mqttConfig.keepAliveInterval;
	data.clientID.cstring = g_mqttConfig.clientID;
	data.username.cstring = g_mqttConfig.username;  // HiveMQ username from config
	data.password.cstring = g_mqttConfig.mqPassword;  // HiveMQ password from config
	...
	s_client = &c;
	//Subscribe mqtt topic (use configured topic or default to 'on/update')
	if (strlen(g_mqttConfig.topic) < 3) {
		memset(g_mqttConfig.topic, 0, sizeof(g_mqttConfig.topic));
		strcpy(g_mqttConfig.topic, "on/update");  // Default topic for HiveMQ
	}
